...
// resolves, it returns a zero BinaryCandidate and false, and the caller
// must treat the language as missing its Language Server.
func ProbeBinaries(spec LanguageSpec) (BinaryCandidate, bool) {
	for _, candidate := range spec.Binaries {
		if _, err := exec.LookPath(candidate.Name); err == nil {
			return candidate, true
		}
	}
	return BinaryCandidate{}, false
}
