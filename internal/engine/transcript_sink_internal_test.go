...
// TestSinkFlush_PersistsAfterContextCancellation is issue 454: the sink
// persists on a cancel-immune context, so a debounced flush of a
// post-cancellation event lands in storage without waiting for Close. A run
// cancelled before it streams anything then keeps its diagnostic fallback.
