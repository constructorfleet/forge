...
func (a *Approver) ApproveSpec(ctx context.Context, featureID string) (string, error) {
	...
	return a.approveSpec(ctx, featureID, spec)
}
...
func (a *Approver) ApproveTicketPlan(ctx context.Context, featureID string) (TicketPlanApproval, error) {
	...
	if err != nil {
		return TicketPlanApproval{}, fmt.Errorf("planningapprove: load ticket plan for feature %s: %w", featureID, err)
	}
	if tp == nil {
		return TicketPlanApproval{}, fmt.Errorf("planningapprove: no ticket plan found for feature %s", featureID)
	}
	return a.approveTicketPlan(ctx, featureID, tp)
}
