...
// giteaComment is the subset of Gitea's issue-comment JSON shape Client
...
// GetComments fetches all comments on an Issue, oldest first (Gitea's native
// ordering), normalized to tracker.Comment. It requests the maximum page size
// and follows the Link "next" header across every page, accumulating results —
// otherwise only the first page of comments would be returned and the newest
// comments, which are most likely to carry current instructions or a
// needs-info signal (see CONTEXT.md "Needs-info resume flow"), would be
// silently dropped.
func (c *Client) GetComments(ctx context.Context, id string) ([]tracker.Comment, error) {
	...
	url := c.baseURL + c.issuePath(number, "/comments") + "?limit=50"
...
// Gitea's create-comment response — in particular the author login and
...
func (c *Client) AddComment(ctx context.Context, id string, body string) (tracker.Comment, error) {
	...
	var resp giteaComment
	if err := c.do(ctx, http.MethodPost, c.issuePath(number, "/comments"), reqBody, &resp); err != nil {
		return tracker.Comment{}, err
	}
