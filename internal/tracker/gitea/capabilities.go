...
// CreateChangeRequest adapts the neutral SCM capability to Gitea's existing
// pull-request creation behavior. The neutral request is mapped field by field
// rather than whole-struct converted so the neutral ChangeRequestRequest and
// the provider-native PullRequestRequest can evolve independently.
...
// GetChangeRequestMergeStatus adapts the neutral SCM capability to Gitea's
...
// GetChecks adapts the neutral CI capability to Gitea's existing pull-request
...
// GetReviews adapts SCM's optional neutral review sub-capability to Gitea's
