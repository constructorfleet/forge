...
// Invoke records the call and returns the next programmed outcome for key,
// per fake.OutcomeQueue's consume/repeat/default rules.
func (f *FakeBackend) Invoke(_ context.Context, key, prompt string) (string, error) {
	f.mu.Lock()
	f.invocations = append(f.invocations, Invocation{Key: key, Prompt: prompt})
	f.mu.Unlock()

	return f.outcomes.Next(key)
}
