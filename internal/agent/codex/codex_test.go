...
func TestAdapter_ExecuteDelegatesToCodexCLI(t *testing.T) {
...
func TestAdapter_ExecuteSanitizesEnv(t *testing.T) {
	...
	t.Setenv("FORGE_CODEX_TEST_UNRELATED", "should-not-pass")
	...
	for _, e := range call.env {
		if e == "OPENAI_API_KEY=secret" {
			foundKey = true
		}
		if e == "FORGE_CODEX_TEST_UNRELATED=should-not-pass" {
			t.Fatalf("env leaked unrelated variable: %v", call.env)
		}
	}
	if !foundKey {
		t.Fatalf("env = %v, want OPENAI_API_KEY passed through", call.env)
	}
}
