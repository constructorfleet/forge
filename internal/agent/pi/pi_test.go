...
func TestAdapter_ExecuteSanitizesEnv(t *testing.T) {
	t.Setenv("PI_API_KEY", "secret")
	t.Setenv("FORGE_PI_TEST_UNRELATED", "should-not-pass")
	...
	for _, e := range call.env {
		if e == "PI_API_KEY=secret" {
			foundKey = true
		}
		if e == "FORGE_PI_TEST_UNRELATED=should-not-pass" {
			t.Fatalf("env leaked unrelated variable: %v", call.env)
		}
	}
	if !foundKey {
		t.Fatalf("env = %v, want PI_API_KEY passed through", call.env)
	}
}
