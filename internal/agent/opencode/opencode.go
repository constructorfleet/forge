// Package opencode implements an Agent Adapter (see CONTEXT.md "Agent
// Adapter") that invokes the opencode CLI as a subprocess. It shares its
...
// opencode-specific configuration (executable name, invocation flags, and
...
// Runner executes one opencode CLI invocation. See clicommon.Runner for the
...
// defaultExecutable is the opencode CLI binary name used when
// Adapter.Executable is empty.
...
// defaultAuthEnvVars is the standard set of opencode auth variables
...
// Adapter is a production Agent Adapter that invokes the opencode CLI as a
...
type Adapter struct {
	...
	// "opencode" if Executable is empty) for real.
	Runner Runner

	// Executable is the path or name of the opencode CLI binary. Defaults
	// to "opencode" if empty.
	Executable string
