...
**Dependency**:
A directed relationship indicating that one Issue must complete before another can begin. Dependencies form a DAG; cycles are errors. A Dependency is satisfied when the prerequisite Issue is closed, or when its merged PR is reachable from the applicable base — not when implementation is locally complete, and not merely when CI goes green. An open prerequisite remains unsatisfied. A Dependency governs both *when* the dependent Issue may run and *what repository state* it runs against: a dependent with one or more Dependencies within the Execution set starts from its Dependencies' resulting branches (a single Dependency's branch directly, or an Integration Branch when there is more than one), pinned to a commit SHA, not the Execution's original base branch. See ADR 0018 for the pinned-base and restack model.

**Integration Branch**:
A synthetic branch (`forge/integration/<issue>`) built by merging several Dependencies' branches together for an Issue with more than one Dependency, so that Issue's Worker starts from a repository state containing all of them. Recomputed from scratch each time it's needed; a merge conflict between Dependencies aborts and surfaces deterministically rather than dropping one side. The dependent Issue's base captures this branch's commit SHA at build time, not the branch name, so the base stays valid after the branch is later deleted.

**Dependency Source**:
Where a Dependency relationship originates. On GitHub the canonical source is the tracker's native "blocked by" issue relationships when the host exposes them, falling back to the `## Dependencies` block in the issue body otherwise (see ADR 0003). On GitLab the canonical source is the tracker's native `is_blocked_by` issue links when the instance and the project tier expose them, with the same body-block fallback (see ADR 0027). On Linear the source is always the tracker's native `blocks` issue relations; Linear has no tiered gate on this feature, so there is no body-block fallback (see ADR 0028). `.forge.yaml` overrides take precedence over all of these when present. The Scheduler consumes normalized Dependencies regardless of source.

**External Issue**:
An Issue referenced as a Dependency but not included in the Execution set. Loaded into the DAG as an observed node — tracked for satisfaction but never executed. Forge does not automatically add External Issues to the Execution. Satisfaction is checked by the prerequisite's closed state; for merged prerequisites, merged code must also be reachable from the applicable base.
...
**Merge Conflicts**:
The CI Supervisor detects when a pull request can no longer be merged into its base branch. Forge first attempts the narrow safe shape defined by ADR 0017: an isolated, Git-clean replay of the pull request branch onto the current target tip, validated before publication and restored on failure. When the replay refuses (an overlapping hunk, add/add, or any other Git stop condition), the conflict is a repairable CI failure (ADR 0017, amended): the Issue routes to `CI_FAILED` and the ordinary repair loop re-runs the Worker against the refreshed target tip so the Agent reconciles the branch by merging the target in. `NEEDS_INFO` with a structured comment is reserved for the genuinely ambiguous or ownership-lost cases: a refused precondition, lease mismatch, gate failure on the candidate, or a failed post-publication restoration.
...
**Retry Budget**:
Separate counters for gate failures, review rejections, CI failures, and provider-limit stops. Each has its own configurable ceiling. Every repair — whether from gate failure, review rejection, or CI failure — must rerun the full Quality Gate set before proceeding. The CI counter (`retry.ci`) is the post-PR repair budget: it bounds a failed required check, an actionable review-requesting-changes repair, and a merge-conflict repair (ADR 0017, amended — an unresolved conflict is a repairable CI failure), since all three re-enter the same repair loop from CI_FAILED and exhaust into FAILED. It does not bound NEEDS_INFO detours (ambiguous multi-reviewer feedback, an INCONCLUSIVE pre-commit Review, a pre-commit Review that stays CHANGES_REQUIRED once its own counter is exhausted, or a restack failure, which consumes the CI budget but rests at NEEDS_INFO per ADR 0018) — those require human input, not another repair attempt. The provider-limit counter (`retry.provider_limit`) bounds an external transient condition rather than the quality of the Agent's work, so it is independent of the other three: a provider rate limit must never spend the budget a real gate failure needs.
